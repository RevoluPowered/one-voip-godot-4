...
class AmalgamationFile:
    ...
    def add_file(self, relative_input_path, wrap_in_comment=False):
        def add_marker(prefix):
            self.add_text("")
            self.add_text("// " + "/" * 70)
            self.add_text("//  of content of file: " % (prefix, relative_input_path.replace("\\", "/")))
            self.add_text("// " + "/" * 70)
            self.add_text("")
...
def amalgamate_source(source_top_dir=None, target_source_path=None, header_include_path=None):
    ...
    target_forward_header_path = os.path.join(os.path.dirname(target_source_path), forward_header_include_path)
