...
class BuildDesc:
    ...
    def merged_with(self, build_desc):
        ...
        """
        return BuildDesc(
            self.prepend_envs + build_desc.prepend_envs,
...
class BuildData:
    ...
    def _execute_build_subprocess(self, cmd, env, log_path):
        process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=self.work_dir, env=env)
        ...
        if "generators" in axis:
            for generator_data in axis["generators"]:
                for generator in generator_data["generator"]:
                    build_desc = BuildDesc(generator=generator, prepend_envs=generator_data.get("env_prepend"))
